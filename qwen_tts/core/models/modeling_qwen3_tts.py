...
from .emotion_projector import EmotionProjector
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Qwen3TTSConfig):
        ...
        # Emotion projector: enabled only when explicitly requested via config so that
        # baseline checkpoints load identically (no extra params, no shape changes).
        # Output dim matches speaker embedding (LM hidden_size). Zero-init keeps
        # speaker_emb + projector(emo) bit-exact with baseline at training start.
        if getattr(config, "use_emotion_projector", False):
            self.emotion_projector = EmotionProjector(
                emotion_dim=getattr(config, "emotion_dim", 1024),
                target_dim=self.config.speaker_encoder_config.enc_dim,
            )
        else:
